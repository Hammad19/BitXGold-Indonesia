...
import UsdtTab from "../Trading/Future/UsdtTab";
...
const DepositHistory = () => {
  ...
  let paggination = Array(
    Math.ceil(
      requests.filter((item) => item.token_name === "BXG").length / sort
    )
  )
    .fill()
    .map((_, i) => i + 1);
  ...
  const FetchData = async () => {
    ...
    try {
      if (state.auth.auth.isAdmin === true) {
        const { data } = await axiosInstance
          .get("/api/depositcrypto")
          .catch((err) => {
        ...
        const { data } = await axiosInstance
          .get("/api/depositcrypto/" + state.auth.userDetails.id)
          .catch((err) => {
            toast.error(err.response.data.message, {
              style: { minWidth: 180 },
              position: "top-center",
          ...
          <div className="card">
            <Tab.Container defaultActiveKey="All">
              <div className="card-header border-0 pb-2 flex-wrap">
                <h4 className="heading">{t("deposit_history")}</h4>
                <>
                      ...
                      <div
                        ...
                        <table
                          ...
                          <tbody>
                            {}
                            {requests
                              ?.filter((item) => item.token_name === "BXG")
                              .map((item, index) => (
                                <tr key={index}>
                                  <td>{index + 1}</td>
                        ...
                        <div className="d-sm-flex text-center justify-content-between align-items-center mt-3 mb-3">
                          {requests.filter((item) => item.token_name === "BXG")
                            ?.length > 0 ? (
                            <div className="dataTables_info">
                              {t("showing")} {activePag.current * sort + 1}{" "}
                              {t("to")}{" "}
                              {requests.filter(
                                (item) => item.token_name === "BXG"
                              ).length >
